        ...
        for(int k = 0; k < depth-1; k++){
            ...
            */
            ...
            if((lookupCube.X.size()> 0) && (lookupCube.Y.size() > 0) && (lookupCube.Z.size() > 0)){
                for (int a = 0; a < lookupCube.X.size(); a++) {
                    if (lookupCube.X.at(a).first == 0) {
                        ...
                        xyz[0]=interpolate(cube.verticies[7],cube.verticies[6],cube.values[7],cube.values[6]);
                    }
            ...
            //x only
                ...
                xyz[2] = Point{i,j+1,k};
                xyz[2]= interpolate(cube.verticies[3],cube.verticies[2],cube.values[3],cube.values[2]);
            ...
            /////////////
                ...
                if (lookupCube.Y.at(0).first == 0) {
                    if (lookupCube.Z.at(0).first == 0) {
                        Triangle temp;
                        //triangle 1
                        xyz[0] = Point{i,j,k};
                        xyz[0]= interpolate(cube.verticies[0],cube.verticies[4],cube.values[0],cube.values[4]);
                        ...
                        xyz[2] = Point{i+1,j+1,k};
                        xyz[2]= interpolate(cube.verticies[0],cube.verticies[3],cube.values[0],cube.values[3]);
                        temp = Triangle{xyz[0],xyz[1],xyz[2]};
                    ...
                    if (lookupCube.Z.at(0).first == 0) {
                        ...
                        //triangle 1
                        xyz[0] = Point{i,j,k};
                        xyz[0]= interpolate(cube.verticies[2],cube.verticies[6],cube.values[2],cube.values[6]);
                        xyz[1] = Point{i+1,j,k};
                        xyz[1]= interpolate(cube.verticies[3],cube.verticies[7],cube.values[3],cube.values[7]);
                        xyz[2] = Point{i+1,j+1,k};
                        xyz[2]= interpolate(cube.verticies[0],cube.verticies[3],cube.values[0],cube.values[3]);
            ...
            else if (!lookupCube.Y.size() && lookupCube.X.size() && lookupCube.Z.size())  {
            if (lookupCube.X.at(0).first == 0) {
                if (lookupCube.Z.at(0).first == 0) {
                    Triangle temp;
                    //triangle 1
                    xyz[0] = Point{i,j,k};
                ...
                if (lookupCube.Z.at(0).first == 0) {
                    ...
                    xyz[1] = Point{i+1,j,k};
                    xyz[1]= interpolate(cube.verticies[2],cube.verticies[3],cube.values[2],cube.values[3]);
                    xyz[2] = Point{i+1,j+1,k};
                    xyz[2]= interpolate(cube.verticies[0],cube.verticies[3],cube.values[0],cube.values[3]);
                    temp = Triangle{xyz[0],xyz[1],xyz[2]};
                    triangles.push_back(temp);
            ...
            else if (!lookupCube.Z.size() && lookupCube.X.size() && lookupCube.Y.size()) {
                if (lookupCube.X.at(0).first == 0) {
                    if (lookupCube.Y.at(0).first == 0) {
                        Triangle temp;
                        //triangle 1
                        xyz[0] = Point{i,j,k};
                        xyz[0]= interpolate(cube.verticies[2],cube.verticies[3],cube.values[2],cube.values[3]);
                        xyz[1] = Point{i+1,j,k};
                        xyz[1]= interpolate(cube.verticies[0],cube.verticies[1],cube.values[0],cube.values[1]);
                        xyz[2] = Point{i+1,j,k+1};
                        xyz[2]= interpolate(cube.verticies[0],cube.verticies[4],cube.values[0],cube.values[4]);
                        temp = Triangle{xyz[0],xyz[1],xyz[2]};
                        triangles.push_back(temp);
                        ...
                        xyz[1]= interpolate(cube.verticies[2],cube.verticies[6],cube.values[2],cube.values[6]);
                        xyz[2] = Point{i+1,j,k+1};
                        xyz[2]= interpolate(cube.verticies[1],cube.verticies[5],cube.values[1],cube.values[5]);
                        temp = Triangle{xyz[0],xyz[1],xyz[2]};
                        triangles.push_back(temp);
                    }
                }
